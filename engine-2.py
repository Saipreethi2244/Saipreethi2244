...
class CellDiff:
    ...
    @property
    def content_match_pct(self) -> float:
        """ADDED: % of all populated content (across BOTH years combined)
        that is identical -- i.e. how much of the sheet's content actually
        rolled forward as-is. Unlike unchanged_ratio (used only inside the
        Roll-Forward Score), this includes new/cleared cells in the
        denominator, so it reflects the sheet's full content footprint,
        not just the matched subset."""
        total = self.total_populated
        return round((self.unchanged / total) * 100, 1) if total else 100.0
...
def summarize_workbook(pair_label: str, sheet_matches: List[SheetMatch],
    ...
    # ADDED: workbook-level Content Match % -- average, across sheets, of
    # how much of each sheet's content (populated cells on either side)
    # rolled forward unchanged. High % = mostly rolled forward as-is;
    # low % = heavily edited or largely new content this year.
    avg_content_match_pct = (
        round(sum(s.cell_diff.content_match_pct for s in sheet_matches) / len(sheet_matches), 1)
        if sheet_matches else 0.0
    )
    ...
    return {
        ...
        "Content Match %": avg_content_match_pct,
...
def rollforward_sheet_names(pair_label: str, sheet_matches: List[SheetMatch]) -> List[dict]:
    ...
    return [{
        ...
        "Content Match %": s.cell_diff.content_match_pct,  # ADDED
...
def ipo_sheet_detail_table(pair_label: str, sheet_matches: List[SheetMatch],
    ...
    for name in _visible_sheetnames(wb_curr):
        ...
        rows.append({
            ...
            "Content Match %": sm.cell_diff.content_match_pct if (sm and rolled_forward) else "",  # ADDED
        })
    return rows
